...
--     ・study_notice_enabled は学習中のお知らせ（要件12章 / UC 12.2）。ポモドーロの
--       フェーズ境界・全ループ完了・目標到達・5:00自動終了をまとめて1つの設定で扱う。
--       出来事の時刻は active_session と学習日の実績合計から都度算出するため、
--       予約状態を持つ列は設けない
-- =====================================================================
CREATE TABLE notification_setting (
    ...
    study_notice_enabled  INTEGER NOT NULL DEFAULT 0 CHECK (study_notice_enabled IN (0, 1)),
    updated_at            TEXT    NOT NULL DEFAULT (datetime('now'))
);
