...
const MODULE_DOCS = {
  ...
  "night-photo.test.ts": {
    title: "その夜の写真の帰属・保存名・撮影時刻の表示",
    target: "src/lib/night-photo.ts",
    requirements: "要件2.6（今夜の写真）／要件4.1（日別記録閲覧）",
    why: "写真がどの夜の記録になるかは画面を見ても分からない。帰属は「撮り始めた時刻」の学習日で、保存が完了した時刻では判定しないため、5:00の境界や日をまたいだ撮影でずれると、昨夜の1枚が今夜に付く形で記録が静かに壊れる。あわせて、撮り直しのたびに保存名が変わることも固定する（同じ名前で上書きすると、表示側が古い画像をキャッシュしたまま更新されない）。撮影時刻の表示は、日をまたいで撮った写真が前日の夜に並ぶ理由をユーザーへ示す唯一の手掛かりであり、欠けると記録が読み違えられる。",
  },
  ...
  "calendar.test.ts": {
    title: "カレンダー表示（グリッド・最頻の算出・月の完了判定）",
    target: "src/lib/calendar.ts",
    requirements: "要件4.1（日別記録閲覧）／要件4.2（月次サマリー）",
    why: "月グリッドの生成は月初の曜日やうるう年でずれやすく、画面を見ても正しさが分からない。最頻（最も多かった感情・天気）の算出は、同数時の tie-break で挙動が変わる。ふりかえりの一言を出す条件となる「完了した過去月か」の判定は、当月・月末当日・翌月・年またぎの境界を取り違えると進行中の月にメッセージが出てしまう。どれも表示が静かに狂うため境界を固定する。",
  },
  "monthly-review.test.ts": {
    title: "月のふりかえり（傾向の分類・文面の選択）",
    target: "src/lib/monthly-review.ts",
    requirements: "要件4.2（月次サマリー）の拡張／要件7.1（NPCメッセージ）とは別系統",
    why: "感情のカテゴリ比率（ポジ/ニュートラル/ネガ）から傾向のトーンを決める分類は、しきい値（0.45/0.30/0.75）や同数の tie-break で結果が変わり、崩れると『たたえる相手・トーン』がずれる。文面は月ごとに固定しつつ飽きさせないため年月から決定的に選ぶ。画面を見ても正しさが分からないため、境界と選択の決定性を固定する。",
  },
  "audio.test.ts": {
    title: "音量の換算・ダッキング・BGMのプレイリスト/シャッフル",
    target: "src/lib/audio.ts",
    requirements: "要件9（BGM・音楽プレイリスト）／要件3.3（終了演出）／要件10.4（音量設定）",
    why: "音量は「設定値0〜100」と「プレイヤー0.0〜1.0」の2つの世界があり、変換がずれても耳では気づけない。とくに「音量0の音は再生処理自体を行わない」は要件の明示ルールで、0と1の境目を取り違えると鳴るはずの音が消える（またはその逆）。再生キューの生成（ソース別の絞り込み・シャッフルで全曲ちょうど1回ずつ・一巡後の重複回避）も曲の取りこぼしや重複が画面を見て分からないため、境界を固定する。",
  },
  ...
  "video-variant.test.ts": {
    title: "背景のループ動画のパターン抽選（学習日ごと）",
    target: "src/lib/video-variant.ts",
    requirements: "要件2.2（街探索・背景のループ動画）／要件0章（学習日）",
    why: "同じ枠に動きだけが違う動画を複数登録できるため、どれを流すかを決める必要がある。ここで乱数を使うと、背景は毎分の時刻更新・画面回転・レベルアップのたびに再評価されるため、見ている前で動画が別のものへ切り替わる。静かな世界観をいちばん損なう壊れ方でありながら、画面を一瞬見ただけでは「たまたま切り替わった」のか判別できない。学習日をシードにした決定的な選択であること・学習日が変われば選び直されること・添字が範囲内に収まることを固定する。",
  },
  ...
  "event-notice.test.ts": {
    title: "予定通知の組み立て（1週間前・前日）",
    target: "src/lib/event-notice.ts",
    requirements: "要件4.3（カレンダーの予定・メモ）",
    why: "予定日ごとに「1週間前」「前日」の12:00へ通知を組む。すでに過ぎた発火は捨てる（1週間を切って登録したら前日だけ）。発火時刻・件数・境界（前日12:00ちょうどは出さない・月跨ぎの前日・当日/過去は0件）は画面を見ても分からず、間違えると誤った時刻に通知が飛ぶためテストで固定する。",
  },
  "study-notice.test.ts": {
    title: "学習中のお知らせ（バックグラウンド通知）の予約",
    target: "src/lib/study-notice.ts",
    requirements:
      "要件12章（システム通知）／要件3.1（ポモドーロ）／要件3.2（5:00自動終了）／要件6.2（目標達成）／UC 12.2",
    why: "アプリを離れているあいだに起きる4種の出来事（作業⇄休憩の切り替わり・全ループ完了・目標到達・5:00自動終了）を、OSへ先回りして予約する。発火するのは何十分も後で画面を見ても正しさが分からない。とくに目標到達は「その学習日の保存済み実績＋進行中の実績」が一日の学習目標時間に達する時刻を逆算するうえ、ポモドーロでは休憩の分だけ後ろへずれるため間違えやすい。境界（一時停止した分のずれ・5:00以降の切り捨て・作業時間ちょうどの扱い・セッション中に届かない場合）を取り違えると、休憩が終わっても知らせが来ない／目標に届いたのに気づけない／夜が明けたのに分からない形で壊れるため、テストで固定する。休憩提案（5.1）の基準に引きずられないことも、通知が事実上出なくなる壊れ方を防ぐために固定する。",
  },
