...
const MODULE_DOCS = {
  ...
  "pomodoro-notice.test.ts": {
    title: "ポモドーロの切り替わり通知の予約",
    target: "src/lib/pomodoro-notice.ts",
    requirements: "要件12章（システム通知）／要件3.1（ポモドーロ）／UC 12.2",
    why: "アプリを離れているあいだの作業⇄休憩の切り替わりと全ループ完了を、OSへ先回りして予約する。発火するのは何十分も後で画面を見ても正しさが分からず、境界（一時停止した分のずれ・5:00自動終了以降の切り捨て・繰り返し1回では切り替わりが無く完了のみ）を取り違えると、休憩が終わっても知らせが来ない／休憩中に作業へ戻される／終わったのに気づけない形で壊れ、休憩を挟むというポモドーロの目的自体が果たせなくなるためテストで固定する。",
  },
