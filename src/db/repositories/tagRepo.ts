...
/** 有効なタグ（標準＋マイタグ）の件数（上限判定用。論理削除済みは含めない） */
export async function countActiveTags(): Promise<number> {
  const db = await getDatabase();
  const row = await db.getFirstAsync<{ count: number }>(
    "SELECT COUNT(*) AS count FROM study_tag WHERE is_active = 1",
  );
  return row?.count ?? 0;
}
...
/**
 * タグを登録する（要件3.4）。新規はマイタグ（is_custom=1）として作る。
 *
...
export async function createMyTag(
  ...
  if (existing) {
    // 削除済みのタグ（標準・マイタグ問わず）と同名 → 新規作成せず復活させる
    if (existing.is_active === 0) {
      if ((await countActiveTags()) >= LIMITS.MYTAG_MAX) {
...
/** 管理画面（S10）に出す有効なタグ一覧（標準＋マイタグ。要件10.9。論理削除済みは除く。標準が先） */
export async function listManagedTags(): Promise<StudyTag[]> {
  const db = await getDatabase();
  return db.getAllAsync<StudyTag>(
    `SELECT * FROM study_tag
      WHERE is_active = 1
...
/**
 * タグの名称を変更する（要件10.9）。標準タグ・マイタグとも変更できる。
 *
 * 名前の形式（必須・20文字以内）は validateTagName で先に検証すること。
 * 重複判定は「自分以外に同名の行があるか」で行い、標準・マイタグ・論理削除済みを含めて弾く。
...
export async function renameTag(
  ...
  await db.runAsync("UPDATE study_tag SET name = ? WHERE id = ?", name, id);
  const tag = await db.getFirstAsync<StudyTag>(
    "SELECT * FROM study_tag WHERE id = ?",
    id,
  );
  if (!tag) throw new Error("タグの更新に失敗しました");
  return { ok: true, tag };
}

/**
 * タグを論理削除する（要件10.9）。標準タグ・マイタグとも削除できる。以後の選択肢から
 * 外れるが、過去の学習記録では表示され続ける（is_active=0）。上限20件のカウントにも数えない。
 */
