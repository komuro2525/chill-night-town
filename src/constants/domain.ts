...
/**
 * 夜の天気アルバムの見え方の段階。要件4.2 / 4.4。
 *
 * 学習した夜の数（その夜に学習したかどうかの二値）で決まる。天気の種類数は
 * 条件にしない（集める行為を強いないため）。月次はその月の夜数で決まり月ごとに
 * 変わるが、通算は一度到達した段階を戻さない。
 * 月次のしきい値が低いのは、1か月が最大31夜しかなく、通算と同じ値では
 * 3段階目へ到達できないためである。
 */
export const ALBUM_DECOR = {
  /** 月次サマリー（その月に学習した夜の数） */
  MONTHLY: { STAGE2: 5, STAGE3: 15 },
  /**
   * 通算のふりかえり（全期間で学習した夜の数）。
   * 20夜=1〜2か月の継続、60夜=3〜4か月の継続に相当する。これ以上高くすると、
   * 実データでは到達する前にアプリを使う期間のほうが終わり、演出が働かない。
   */
  OVERALL: { STAGE2: 20, STAGE3: 60 },
} as const;
